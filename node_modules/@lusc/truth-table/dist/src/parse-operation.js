...
import { TokenType, tokenize } from './tokenize.js';
import { validate } from './validate/validate.js';
const toOriginalString = (input) => {
...
const parseNot = (input) => {
    ...
    if (first.type !== TokenType.operator || first.operator !== Operator.not) {
        throw new IndexedError(`Expected "${OperatorSymbols.not}".`, first.from, first.to);
    }
    return {
        type: 'operator',
...
const parseGroup = (input) => {
    ...
    if (!hasOperator(input)) {
        throw new IndexedError(`Expected "${toOriginalString(input)}" to have an operator.`, input[0].from, input.at(-1).to);
    }
    if (input.length === 1) {
        const item = input[0];
...
const parseOperationInternal = (input) => {
    if (input.length === 0) {
        throw new Error('Unexpected empty input at _parseOperation.');
    }
    if (input.length === 1) {
    ...
    const lastItems = [input.pop()];
    let secondToLast;
    while ((secondToLast = input.at(-1))
    ...
    if (operatorArray.length !== 1) {
        throw new IndexedError(`Expected operator, got "${toOriginalString(operatorArray)}".`, operatorArray[0].from, operatorArray.at(-1).to);
    }
    if (operator.type !== TokenType.operator) {
        throw new IndexedError(`Expected operator, got type "${operator.type}" with value "${toOriginalString(operator)}"`, operator.from, operator.to);
    }
    if (operator.operator === Operator.not) {
        throw new IndexedError(`Unexpected "${toOriginalString(operator)}".`, operator.from, operator.to);
    }
    return {
        type: 'operator',
 ...
 */
export const parseOperation = (raw) => {
