...
export const validateMatchedParens = (input) => {
    ...
    for (const item of input) {
        if (item.type === TokenType.parens) {
            if (item.parensType === 'open') {
                ...
                throw new IndexedError(`Unmatched parenthesis at position ${item.from}.`, item.from, item.from + 1);
            }
        }
    }
    const last = openingParens.pop();
    if (last !== undefined) {
        throw new IndexedError(`Unmatched parenthesis at position ${last}.`, last, last + 1);
    }
};
//# sourceMappingURL=validate-matched-parens.js.map